...
check_dependencies() {
    ...
    if ! command -v python3 >/dev/null 2>&1; then
        print_message "Python 3 is not installed. Please install it with: sudo apt install python3 python3-venv python3-pip python3-full" "$RED"
        exit 1
    fi
    ...
    if [ -f /etc/debian_version ]; then
        if ! dpkg -l | grep -q python3-venv; then
            print_message "The python3-venv package is not installed. Installing it now..." "$YELLOW"
            sudo apt update && sudo apt install -y python3-venv python3-full
            if [ $? -ne 0 ]; then
                print_message "Failed to install python3-venv. Please install it manually: sudo apt install python3-venv python3-full" "$RED"
                exit 1
            fi
        fi
    fi
...
create_application() {
    print_message "Creating application files..." "$BLUE"
    
    # Create main Python file
    cat > "$INSTALL_DIR/etlegacy_manager.py" << 'EOF'
import os
import sys
import logging
import requests
...
EOF
    ...
    print_message "Application files created successfully." "$GREEN"
}
